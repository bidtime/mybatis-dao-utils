...
type
  ...
  public
    ...
    class function oneline(const S: string): string;
    class function RoundFloat(const v: double; const n: integer): double; static;
    class function getPercents(const tc, curc: integer; const dot: integer=1): string; static;
    class function getPercent(const tc, curc: integer; const dot: integer=1): double; static;
end;
...
class function TCharUtils.oneline(const S: string): string;
var sql: string;
begin
  sql := S.Replace(#13, '');
  Result := sql.Replace(#10, '');
end;
...
class function TCharUtils.RoundFloat(const v: double; const n: integer): double;
var
  s: string;
  ef: extended;
begin
  s := '#.' + StringOfChar('0', n);
  ef := StrToFloat( FloatToStr(v) );                  //·ÀÖ¹¸¡µãÔËËãµÄÎó²î
  Result := StrToFloat( FormatFloat(s, ef) );
end;

class function TCharUtils.getPercent(const tc, curc: integer; const dot: integer): double;
var percent: double;
begin
  if tc = 0 then begin
    percent := 0.0;
  end else begin
    percent := TCharUtils.RoundFloat(( curc / tc ) * 100, dot);
  end;
  Result := percent;
end;

class function TCharUtils.getPercents(const tc, curc: integer; const dot: integer): string;
begin
  Result := Format('%.1f', [getPercent(tc, curc, dot)]) + '%';
end;
